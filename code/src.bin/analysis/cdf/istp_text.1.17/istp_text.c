...
/* 
 $Log: istp_text.c,v $
 Revision 1.14  2005/08/05 19:54:50  barnes
 Added command line option to dump the possible command line options.

 Revision 1.13  2005/07/18 17:51:42  barnes
 Modification to use the h0 ACE MFI files.

 Revision 1.12  2004/05/04 19:18:32  barnes
 Renamed the type header to avoid confusion.

 Revision 1.11  2004/04/27 18:32:52  barnes
 Audit - Enforced warning set to "all".

 Revision 1.10  2003/08/06 21:35:09  barnes
 Changes to library calls.

 Revision 1.9  2003/03/09 17:29:11  barnes
 Changed CDF header name.

 Revision 1.8  2003/02/26 18:41:55  barnes
 Modification to deal with base libraries.

 Revision 1.7  2001/06/27 20:57:21  barnes
 Added license tag

 Revision 1.6  2001/01/29 18:12:10  barnes
 Added Author Name

 Revision 1.5  2000/08/16 22:09:10  barnes
 Changed return type of main().
 Revision 1.4  2000/08/13 20:00:40  barnes
 Corrected name of environment variable.

 Revision 1.3  2000/08/13 19:59:49  barnes
 Added environment variable ISTP_PATH.

 Revision 1.2  2000/08/13 18:08:36  barnes
 Added the option of including the spacecraft position or not.
  ...
  if (mfi==1) {
    ...
    if (fptr->cnt !=0) {
      for (i=0;i<fptr->cnt;i++) {
        ...
        status=acemfi_imf(id,&imf,stime,etime);
        CDFclose(id);
      }
      free_locate(fptr);
    } else {
...
int main(int argc,char *argv[]) {
 ...
 if ((etime-stime)>=(24*3600)) dflg=1;
  ...
  for (i=0;i<pnum;i++) {
    ...
    fprintf(stdout,"%.4d %.2d %.2d %.2d %.2d %#8.4g",yr,mo,dy,hr,mt,sc); 
    if (pos.cnt !=0) {
      int x;
      x=find_pos(&pos,ptme[i]);
      if (mode==0) fprintf(stdout,"%#12.4g %#12.4g %#12.4g",pos.PGSM[3*x],
                          pos.PGSM[3*x+1],pos.PGSM[3*x+2]);
      else fprintf(stdout,"%#10.4g %#10.4g %#10.4g",pos.PGSE[3*x],
   ...
   if (imf.cnt !=0) {
      ...
      if (mode==0) fprintf(stdout,"%#12.4g %#12.4g %#12.4g",imf.BGSMc[3*x],
                          imf.BGSMc[3*x+1],imf.BGSMc[3*x+2]);
      else fprintf(stdout,"%#10.4g %#10.4g %#10.4g",imf.BGSEc[3*x],
   ...
   if (plasma.cnt !=0) {
      ...
      if (mode==0) fprintf(stdout,"%#12.4g %#12.4g %#12.4g %#12.4g",
                           plasma.pre[x],
                           plasma.VGSM[3*x],plasma.VGSM[3*x+1],
                           plasma.VGSM[3*x+2]);
      else fprintf(stdout,"%#10.4g %#10.4g %#10.4g %#10.4g",plasma.pre[x],
