...
/*
 ...
 Revision 1.4  2005/03/22 15:18:30  barnes
 ...
 Revision 1.1  2003/05/01 14:03:11  barnes
 Initial revision
...
int main(int argc,char *argv[]) {
  ...
  if (config.sfname.change.buf !=NULL) {
    if (config.sfname.change.type==1) {
      fp=fopen((config.sfname.change.buf)->buf,"r");
      if (fp !=NULL) {
         while(fgets(lbuf,255,fp) !=NULL) { 
    ...
    } else s=ScriptDecode(script,(config.sfname.change.buf)->buf,
                                 (config.sfname.change.buf)->sze);
  }
