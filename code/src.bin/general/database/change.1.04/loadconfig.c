...
/*
 ...
 Revision 1.3  2004/06/21 21:42:51  barnes
 Added missing header.

 Revision 1.2  2004/04/27 16:50:19  barnes
 Audit - Enforced warning set to "all".

 Revision 1.1  2003/04/03 14:57:26  barnes
 Initial revision
...
int decodesfname(struct XMLDBtable *ptr,struct XMLDBtable *tree,
  ...
  for (i=0;i<ptr->num;i++) {
    e=ptr->element[i];
    if (strcmp(e->name,"change")==0) {
      if (config->sfname.change.buf !=NULL) 
          XMLDBFreeBuffer(config->sfname.change.buf);

      config->sfname.change.type=0;
      for (j=0;j<e->atnum;j++) {
	if ((e->atval[j] !=NULL) && (strcmp(e->atval[j],"file")==0)) 
        config->sfname.change.type=1;
      }
      if (config->sfname.change.type==1) { 
        config->sfname.change.buf=XMLDBCopyBuffer(e->data);
        XMLDBAddBuffer(config->sfname.change.buf,zero,1);
      } else config->sfname.change.buf=EntityDecodeBuffer(entptr,e->data);
      if (config->sfname.change.buf==NULL) break;
    }
  }
  free(entptr);
  if (i<ptr->num) return -1;
