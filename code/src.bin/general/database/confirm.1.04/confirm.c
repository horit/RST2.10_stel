...
/*
 ...
 Revision 1.4  2005/03/22 14:58:54  barnes
 Changed the modifier syntax to agree with show.cgi.
...
int decode_compare(struct XMLDBtable *ptr,struct XMLDBtable *tree,void *data) {
  int i,j,k;
  char *x,*y;
  struct compare *c;
  struct XMLDBelement *e;
...
int main(int argc,char *argv[]) {
  ...
  if (config.sfname.confirm.buf !=NULL) {
    if (config.sfname.confirm.type==1) {
      fp=fopen((config.sfname.confirm.buf)->buf,"r");
      if (fp !=NULL) {
         while(fgets(lbuf,255,fp) !=NULL) { 
    ...
    } else s=ScriptDecode(script,(config.sfname.confirm.buf)->buf,
                                 (config.sfname.confirm.buf)->sze);
  }
