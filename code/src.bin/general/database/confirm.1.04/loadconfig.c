...
/*
 ...
 Revision 1.3  2004/06/21 21:44:05  barnes
 Added missing header.

 Revision 1.2  2004/04/27 16:52:48  barnes
 Audit - Enforced warning set to "all".

 Revision 1.1  2003/04/03 14:58:11  barnes
 Initial revision
...
int decodesfname(struct XMLDBtable *ptr,struct XMLDBtable *tree,
  ...
  for (i=0;i<ptr->num;i++) {
    e=ptr->element[i];
    if (strcmp(e->name,"confirm")==0) {
      if (config->sfname.confirm.buf !=NULL) 
          XMLDBFreeBuffer(config->sfname.confirm.buf);

      config->sfname.confirm.type=0;
      for (j=0;j<e->atnum;j++) {
	if ((e->atval[j] !=NULL) && (strcmp(e->atval[j],"file")==0)) 
        config->sfname.confirm.type=1;
      }
      if (config->sfname.confirm.type==1) { 
        config->sfname.confirm.buf=XMLDBCopyBuffer(e->data);
        XMLDBAddBuffer(config->sfname.confirm.buf,zero,1);
      } else config->sfname.confirm.buf=EntityDecodeBuffer(entptr,e->data);
      if (config->sfname.confirm.buf==NULL) break;
    }
  }
  free(entptr);
  if (i<ptr->num) return -1;
