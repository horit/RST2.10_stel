...
int decodesfname(struct XMLDBtable *ptr,struct XMLDBtable *tree,
  ...
  for (i=0;i<ptr->num;i++) {
    e=ptr->element[i];
    if (strcmp(e->name,"mail")==0) {
      if (config->sfname.mail.buf !=NULL) 
          XMLDBFreeBuffer(config->sfname.mail.buf);

      config->sfname.mail.type=0;
      for (j=0;j<e->atnum;j++) {
	if ((e->atval[j] !=NULL) && (strcmp(e->atval[j],"file")==0)) 
        config->sfname.mail.type=1;
      }
      if (config->sfname.mail.type==1) { 
        config->sfname.mail.buf=XMLDBCopyBuffer(e->data);
        XMLDBAddBuffer(config->sfname.mail.buf,zero,1);
      } else config->sfname.mail.buf=EntityDecodeBuffer(entptr,e->data);
      if (config->sfname.mail.buf==NULL) break;
    }

    if (strcmp(e->name,"mail.mail")==0) {
      if (config->sfname.mailmail.buf !=NULL) 
          XMLDBFreeBuffer(config->sfname.mailmail.buf);

      config->sfname.mailmail.type=0;
      for (j=0;j<e->atnum;j++) {
	if ((e->atval[j] !=NULL) && (strcmp(e->atval[j],"file")==0)) 
        config->sfname.mailmail.type=1;
      }
      if (config->sfname.mailmail.type==1) { 
        config->sfname.mailmail.buf=XMLDBCopyBuffer(e->data);
        XMLDBAddBuffer(config->sfname.mailmail.buf,zero,1);
      } else config->sfname.mailmail.buf=EntityDecodeBuffer(entptr,e->data);
      if (config->sfname.mailmail.buf==NULL) break;
    }
...
int decodemlist(struct XMLDBtable *ptr,struct XMLDBtable *tree,
                   void *data) {
  int i,n;
  struct config *config;
  struct XMLDBelement *e;
  ...
  for (i=0;i<ptr->num;i++) {
    ...
    if (config->mlist.mail==NULL) config->mlist.mail=malloc(sizeof(char *)); 
    else config->mlist.mail=realloc(config->mlist.mail,
                                     sizeof(char *)*(n+1)); 
  
    if (config->mlist.mail==NULL) break;
    config->mlist.mail[n]=getDBstring(e->data);
    if (config->mlist.mail[n]==NULL) break;
    config->mlist.num++;
  }
  if (i<ptr->num) return -1;
...
int decodeconfig(struct XMLDBtable *ptr,struct XMLDBtable *tree,void *data) {
  ...
  for (i=0;i<ptr->num;i++) {
    ...
    } else if (strcmp(e->name,"mlist")==0) {
      XMLdocGetConfigSection(e->data,"mlist","mlist",decodemlist,
                             config);
