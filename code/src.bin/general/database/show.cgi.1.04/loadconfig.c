...
/*
 $Log: loadconfig.c,v $
 Revision 1.3  2004/06/21 21:45:58  barnes
...
int decodesfname(struct XMLDBtable *ptr,struct XMLDBtable *tree,
  ...
  for (i=0;i<ptr->num;i++) {
    e=ptr->element[i];
    if (strcmp(e->name,"top")==0) {
      if (config->sfname.top.buf !=NULL) 
          XMLDBFreeBuffer(config->sfname.top.buf);

      config->sfname.top.type=0;
      for (j=0;j<e->atnum;j++) {
	if ((e->atval[j] !=NULL) && (strcmp(e->atval[j],"file")==0)) 
        config->sfname.top.type=1;
      }
      if (config->sfname.top.type==1) { 
        config->sfname.top.buf=XMLDBCopyBuffer(e->data);
        XMLDBAddBuffer(config->sfname.top.buf,zero,1);
      } else config->sfname.top.buf=EntityDecodeBuffer(entptr,e->data);
      if (config->sfname.top.buf==NULL) break;
    }
    if (strcmp(e->name,"main")==0) {
      if (config->sfname.main.buf !=NULL) 
          XMLDBFreeBuffer(config->sfname.main.buf);

      config->sfname.main.type=0;
      for (j=0;j<e->atnum;j++) {
	if ((e->atval[j] !=NULL) && (strcmp(e->atval[j],"file")==0)) 
        config->sfname.main.type=1;
      }
      if (config->sfname.main.type==1) { 
        config->sfname.main.buf=XMLDBCopyBuffer(e->data);
        XMLDBAddBuffer(config->sfname.main.buf,zero,1);
      } else config->sfname.main.buf=EntityDecodeBuffer(entptr,e->data);
      if (config->sfname.main.buf==NULL) break;
    }

    if (strcmp(e->name,"tail")==0) {
      if (config->sfname.tail.buf !=NULL) 
          XMLDBFreeBuffer(config->sfname.tail.buf);

      config->sfname.tail.type=0;
      for (j=0;j<e->atnum;j++) {
	if ((e->atval[j] !=NULL) && (strcmp(e->atval[j],"file")==0)) 
        config->sfname.tail.type=1;
      }
      if (config->sfname.tail.type==1) { 
        config->sfname.tail.buf=XMLDBCopyBuffer(e->data);
        XMLDBAddBuffer(config->sfname.tail.buf,zero,1);
      } else config->sfname.tail.buf=EntityDecodeBuffer(entptr,e->data);
      if (config->sfname.tail.buf==NULL) break;
    }
