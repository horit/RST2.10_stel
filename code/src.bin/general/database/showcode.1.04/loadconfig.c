...
/*
 ...
 Revision 1.3  2004/06/21 21:47:51  barnes
 Added missing header.

 Revision 1.2  2004/04/27 17:19:44  barnes
 Audit - Enforced warning set to "all".

 Revision 1.1  2003/04/03 15:00:04  barnes
 Initial revision
...
int decodesfname(struct XMLDBtable *ptr,struct XMLDBtable *tree,
  ...
  for (i=0;i<ptr->num;i++) {
    e=ptr->element[i];
    if (strcmp(e->name,"showcode")==0) {
      if (config->sfname.showcode.buf !=NULL) 
          XMLDBFreeBuffer(config->sfname.showcode.buf);

      config->sfname.showcode.type=0;
      for (j=0;j<e->atnum;j++) {
	if ((e->atval[j] !=NULL) && (strcmp(e->atval[j],"file")==0)) 
        config->sfname.showcode.type=1;
      }
      if (config->sfname.showcode.type==1) { 
        config->sfname.showcode.buf=XMLDBCopyBuffer(e->data);
        XMLDBAddBuffer(config->sfname.showcode.buf,zero,1);
      } else config->sfname.showcode.buf=EntityDecodeBuffer(entptr,e->data);
      if (config->sfname.showcode.buf==NULL) break;
    }
  }
  free(entptr);
  if (i<ptr->num) return -1;
