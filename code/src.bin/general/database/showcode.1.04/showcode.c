...
/*
 ...
 Revision 1.4  2005/03/22 15:23:16  barnes
 ...
 Revision 1.1  2003/05/01 14:08:00  barnes
 Initial revision
...
int main(int argc,char *argv[]) {
  ...
  if (config.sfname.showcode.buf !=NULL) {
    if (config.sfname.showcode.type==1) {
      fp=fopen((config.sfname.showcode.buf)->buf,"r");
      if (fp !=NULL) {
         while(fgets(lbuf,255,fp) !=NULL) { 
    ...
    } else s=ScriptDecode(script,(config.sfname.showcode.buf)->buf,
                                 (config.sfname.showcode.buf)->sze);
  }
