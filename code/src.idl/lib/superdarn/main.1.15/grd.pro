...
; ---------------------------------------------------------------
; $Log: grd.pro,v $
; Revision 1.2  2004/11/03 21:30:43  barnes
; Fixed bug in the external read routine that caused a crash when reading
; empty records.
;
; Revision 1.1  2004/11/01 23:05:13  barnes
; Initial revision
;
; ---------------------------------------------------------------
;
...
function GridNativeRead,unit,prm,stvec,gvec
...
;
; CALLING SEQUENCE:
;       status = GridNativeWrite(unit,prm,stvec,gvec)
;
...
function GridNativeWrite,unit,prm,stvec,gvec
...
;
; CALLING SEQUENCE:
;       status = GridNativeGetTime(sclvec)
;
...
function GridNativeGetTime,sclvec
...
function GridNativeSeek,unit,yr,mo,dy,hr,mt,sc,inx,atme=atme
  ...
  if (n_elements(inx) ne 0) then begin
   ...
   repeat begin
    ...
    return,0
     ...
     fptr=0L
     if (n_elements(sclvec) ne 0) then begin
       tfile=GridNativeGetTime(sclvec)
       st=DataMapFreeScalar(sclvec)
       st=DataMapFreeArray(arrvec)
...
;
; CALLING SEQUENCE:
;       status = GridNativePoint(unit,ptr)
;
...
function GridNativePoint,unit,off
...
;
; CALLING SEQUENCE:
;       status = GridExternalRead(unit,prm,stvec,gvec)
;
;       This function reads a single record from the open file with
...
function GridExternalRead,unit,prm,stvec,gvec,lib=lib
  ...
  if (KEYWORD_SET(lib) eq 0) then lib=getenv('LIB_GRDIDL')
...
;
; CALLING SEQUENCE:
;       status = GridExternalWrite(unit,prm,stvec,gvec)
;
;       This function writes a single record to the open file with
;       logical unit number, unit.
;
...
function GridExternalWrite,unit,prm,stvec,gvec,lib=lib
 
  if (KEYWORD_SET(lib) eq 0) then lib=getenv('LIB_GRDIDL')
  s=CALL_EXTERNAL(lib,'GridIDLWrite', $
                  unit,prm,stvec,gvec)
...
;
; CALLING SEQUENCE:
;       status = GridExternalSeek(unit,yr,mo,dy,hr,mt,sc,atme,inx)
;
;       This function searchs for a record in a grdmap file.
;
;       The returned value is zero on success,
...
function GridExternalSeek,unit,yr,mo,dy,hr,mt,sc,inx,atme=atme,lib=lib
  ...
  if (n_elements(inx) ne 0) then begin
    ...
    s=CALL_EXTERNAL(lib,'GridIDLSeek', $
...
;
; CALLING SEQUENCE:
;       status = GridExternalPoint(unit,ptr,lib=library)
;
;       This function sets the file pointer in a grdmap file.
;
;       The returned value is the offset on success,
...
function GridExternalPoint,unit,off,lib=lib

 if (KEYWORD_SET(lib) eq 0) then lib=getenv('LIB_GRDIDL')   
 offl=long(off)
 s=CALL_EXTERNAL(lib,'GridIDLPoint', $
                unit,offl) 
 return, s
end
...
;
; CALLING SEQUENCE:
;       status = GridRead(unit,prm,stvec,gvec,lib=library,/native,/external)
;
;       This function reads a single record from the open file with
;       logical unit number.
;
...
function GridRead,unit,prm,stvec,gvec,lib=lib,native=native,external=external
  ...
  if (mode eq 2) then s=GridExternalRead(unit,prm,stvec,gvec,lib=lib) $
  else s=GridNativeRead(unit,prm,stvec,gvec)
  return, s
end
...
;
; CALLING SEQUENCE:
;       status = GridWrite(unit,prm,stvec,gvec,lib=library,/native,/external)
;
;       This function writes a single record to the open file with
;       logical unit number, unit. 
;
...
function GridWrite,unit,prm,stvec,gvec,lib=lib,native=native,external=external
  ...
  if (mode eq 2) then s=GridExternalWrite(unit,prm,stvec,gvec,lib=lib) $
  else s=GridNativeWrite(unit,prm,stvec,gvec)
  return, s
end
...
;
; CALLING SEQUENCE:
;       status = GridSeek(unit,yr,mo,dy,hr,mt,sc,inx,atme=atme,lib=library,
;                        /native,/external)
;
;       This function searchs for a record in a grdmap file.
;
...
function GridSeek,unit,yr,mo,dy,hr,mt,sc,inx,atme=atme,lib=lib, $
  ...
  if (mode eq 2) then s=GridExternalSeek(unit,yr,mo,dy,hr,mt,sc,inx, $
                                        lib=lib,atme=atme) $
  else s=GridNativeSeek(unit,yr,mo,dy,hr,mt,sc,inx,atme=atme)
  return, s
end
...
;
; CALLING SEQUENCE:
;       unit = GridOpen(lib=library,/native,/external,/read,/write,/update)
;
...
function GridOpen,fname,atme=atme,lib=lib, $
                 native=native,external=external, $
...
;
; CALLING SEQUENCE:
;       status = GridPoint(fitfp,ptr,lib=library)
;
;       This function sets the file pointer in a grdmap file.
;
;
...
function GridPoint,unit,off,lib=lib
  ...
  if (mode eq 1) then return, GridNativePoint(unit,off) $
  else return, GridExternalPoint(unit,off,lib=lib)
