...
; modified by N. Nishitani 2011/07/13
    if count eq 4 then begin
    print,'Old rawacf format file.'
    count_oldrawacf=1
    endif else begin
...
;     prm.lag[0:prm.mplgs,*]=(*(arrvec[arrid[1]].ptr))[*,*]
  if (prm.mplgs gt 0) then begin
     if (count_oldrawacf eq 0) then begin
       prm.lag[0:prm.mplgs,*]=(*(arrvec[arrid[1]].ptr))[*,*]
     endif else begin
