...
/*
 $Log: cgi.c,v $
 Revision 1.2  2004/04/26 20:22:10  barnes
 Audit - Enforced warning set to "all".

 Revision 1.1  2003/05/13 20:45:03  barnes
 Initial revision
...
int CGIInterpretPart(struct CGIdata *ptr,char *buf,int sze) {
  ...
  if ((buf==NULL) && (n>0)) {
    ...
    if (ptr->text.user !=NULL) s=(ptr->text.user)(a,NULL,0,
                                                    ptr->text.data);
  ...
  if (a->type==1) s=1;
  if (ptr->text.user !=NULL) s=(ptr->text.user)(a,buf,sze,ptr->text.data);
