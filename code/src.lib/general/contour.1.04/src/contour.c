...
/*
 $Log: contour.c,v $
 Revision 1.4  2004/06/16 16:32:26  barnes
 Added missing header.

 Revision 1.3  2004/05/04 22:42:30  barnes
 Changed the header name.

 Revision 1.2  2004/04/26 20:37:00  barnes
 Audit - Enforced warning set to "all".

 Revision 1.1  2003/08/06 20:04:32  barnes
 Initial revision
...
struct PolygonData **ContourTransform(int cnum,struct PolygonData **src,
  ...
  for (i=0;i<cnum;i++) {
    ...
    dst[i]=PolygonTransform(src[i],sizeof(float)*2,NULL,trf,data);
  }
  return dst;
} 
...
void ContourModify(int cnum,struct PolygonData **src,
  ...
  if (src==NULL) return;
  for (i=0;i<cnum;i++) {
    if (src[i] !=NULL) PolygonModify(src[i],trf,data);
  }
