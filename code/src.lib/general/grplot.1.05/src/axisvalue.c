...
int GrPlotAxisValueStart(char *name,int atnum,char **atname,char **atval,
  ...
  struct GrPlotAxisValue *ptr;
  struct GrPlotData *plt;

  blk=(struct GrPlotBlock *) data;
  if (blk==NULL) return -1;
  ptr=(struct GrPlotAxisValue *) blk->data;
  ...
  if (strcmp(name,blk->name)==0) {
    ...
    for (i=0;i<atnum;i++) {
      ...
      else if (strcmp(atname[i],"size")==0) ptr->size=atof(atval[i]);
...
int GrPlotAxisValueEnd(char *name,char *buf,int sze,void *data) {
  ...
  if (strcmp(name,blk->name)==0) {
    ...
    PlotXaxisValue(plot,plt->num,ptr->min,ptr->max,ptr->val,
                   ptr->size,ptr->flg,ptr->color,ptr->mask,ptr->width);

    if (strcmp(name,"yaxisvalue")==0) 
    PlotYaxisValue(plot,plt->num,ptr->min,ptr->max,ptr->val,
                   ptr->size,ptr->flg,ptr->color,ptr->mask,ptr->width);
