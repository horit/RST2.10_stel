...
int GrPlotBezierStart(char *name,int atnum,char **atname,char **atval,
  ...
  struct GrPlotBezier *ptr;
  struct GrPlotData *plt;


  blk=(struct GrPlotBlock *) data;
  if (blk==NULL) return -1;
  ptr=(struct GrPlotBezier *) blk->data;
...
int GrPlotBezierEnd(char *name,char *buf,int sze,void *data) {
  ...
  if (strcmp(name,blk->name)==0) {
    ...
    if (ptr->state !=0x1f) return -1;

    if (ptr->off !=NULL) {
      ptr->x1=(ptr->off)->x+ptr->x1*(ptr->off)->w;
      ptr->y1=(ptr->off)->y+ptr->y1*(ptr->off)->h;
      ptr->x2=(ptr->off)->x+ptr->x2*(ptr->off)->w;
      ptr->y2=(ptr->off)->y+ptr->y2*(ptr->off)->h;
      ptr->x3=(ptr->off)->x+ptr->x3*(ptr->off)->w;
      ptr->y3=(ptr->off)->y+ptr->y3*(ptr->off)->h;
      ptr->x4=(ptr->off)->x+ptr->x4*(ptr->off)->w;
      ptr->y4=(ptr->off)->y+ptr->y4*(ptr->off)->h;
 
    }  

    PlotBezier(plot,plt->num,ptr->x1,ptr->y1,ptr->x2,ptr->y2,
	       ptr->x3,ptr->y3,ptr->x4,ptr->y4,
