...
int GrPlotFitImageNameStart(char *name,int atnum,char **atname,char **atval,
  ...
  struct GrPlotFitImageName *ptr;
  struct GrPlotData *plt;
 
  blk=(struct GrPlotBlock *) data;
  if (blk==NULL) return -1;
  ptr=(struct GrPlotFitImageName *) blk->data;
...
int GrPlotFitImageNameEnd(char *name,char *buf,int sze,void *data) {
  ...
  if (strcmp(name,blk->name)==0) {
    ...
    if (plt->image.size==NULL) return -1;
    s=(plt->image.size)(ptr->name,&x,&y,plt->image.data);
    if (s !=0) return -1;

    PlotFitImageName(plot,plt->num,ptr->name,x,y,ptr->mask);
    free(ptr->name);
    free(ptr);
    GrPlotRestore(blk);
    return 0;
