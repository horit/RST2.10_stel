...
int GrPlotValueText(char *buf,int sze,void *data) {
  ...
  struct GrPlotValue *ptr;

  char *tmp;
  int o=0,c=0,i;
  blk=(struct GrPlotBlock *) data;
  if (blk==NULL) return -1;
  ptr=(struct GrPlotValue *) blk->data;
...
int GrPlotValueStart(char *name,int atnum,char **atname,char **atval,
  ...
  if (strcmp(name,blk->name)==0) {
    ...
    for (i=0;i<atnum;i++) {
      if (strcmp(atname[i],"type")==0) {
        if (ptr->type !=NULL) free(ptr->type);
        ptr->type=malloc(strlen(atval[i])+1);
        strcpy(ptr->type,atval[i]);
      }
    }
    return 0;
  }
...
int GrPlotValueEnd(char *name,char *buf,int sze,void *data) {
  ...
  if (strcmp(name,blk->name)==0) {
    ...
    if (ptr->buf==NULL) return -1;
    cnv=ptr->cnv;
    if (ptr->type==NULL) s=(sscanf(ptr->buf,"%lg",ptr->val) !=1);
    else if (cnv->user !=NULL)
	s=(cnv->user)(ptr->type,ptr->buf,ptr->val,cnv->data);
    else *ptr->val=0;
